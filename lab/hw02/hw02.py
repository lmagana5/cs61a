...
def make_repeater(h, n):
    ...
    #There are 3 arguments being passed so you need an inner function.
    def repeater(x):
        for i in range(n):
           x = h(x)
        return x
...
def zero(f):
    return lambda x: x
#A function that takes f and returns x

def successor(n):
    return lambda f: lambda x: f(n(f)(x))
# f(zero(f)(0))
#A function that takes f and returns a function that takes x and returns f of n of f and x
def one(f):
